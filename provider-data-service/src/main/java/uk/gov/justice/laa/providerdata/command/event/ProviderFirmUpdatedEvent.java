 ...
 *
 * <p>Published synchronously within the originating transaction via Spring's {@link
 * org.springframework.context.ApplicationEventPublisher}. Consumed by {@link
 * uk.gov.justice.laa.providerdata.command.event.CommandAuditEventListener} after commit to write a
 * durable audit record.
 *
 * <p>Does not carry the full before/after state — audit listeners can reload from the database if a
 * full snapshot is required in a later phase.
 */
public record ProviderFirmUpdatedEvent(
  ...
  public static ProviderFirmUpdatedEvent of(
    ...
    return new ProviderFirmUpdatedEvent(
        providerFirmGUID, firmNumber, "UpdateProviderFirm", patch, OffsetDateTime.now());
  }
}
