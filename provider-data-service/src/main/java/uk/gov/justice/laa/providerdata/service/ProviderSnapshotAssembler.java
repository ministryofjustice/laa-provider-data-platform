...
public class ProviderSnapshotAssembler {
  ...
  public ProviderFirmChangedSnapshotEventV2Payload assemble(ProviderEntity provider) {
    var lspHeadOffice = providerQueryService.getLspHeadOffice(provider).orElse(null);
    var chambersHeadOffice = providerQueryService.getChambersHeadOffice(provider).orElse(null);
    var advocateOfficeLink = providerQueryService.getAdvocateOfficeLink(provider).orElse(null);
