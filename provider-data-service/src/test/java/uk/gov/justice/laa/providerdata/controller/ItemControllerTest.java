...
class ItemControllerTest {
  ...
  void createItem_returnsCreatedStatusAndLocationHeader() throws Exception {
    ItemRequestBody itemRequestBody =
        ItemRequestBody.builder().name("Item Three").description("Updated item three.").build();
    ...
    mockMvc
        .perform(
            post("/api/v1/items")
                ...
                .content("{\"name\": \"Item Three\", \"description\": \"Updated item three.\"}")
  ...
  void updateItem_returnsBadRequestStatus() throws Exception {
    mockMvc
        ...
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.detail").value("Invalid request content."))
        .andExpect(jsonPath("$.instance").value("/api/v1/items/2"));
