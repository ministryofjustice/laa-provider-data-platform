...
class ProviderFirmControllerTest {
  ...
  @MockitoBean private ProviderFirmUseCase providerFirmCreationService;
  ...
  void getProviderFirm_byGuid_returns200WithProviderDetails() throws Exception {
    ...
    when(officeQueryService.getLspHeadOffice(entity)).thenReturn(Optional.empty());
    when(officeQueryService.getChambersHeadOffice(entity)).thenReturn(Optional.empty());
    when(officeQueryService.getAdvocateOfficeLink(entity)).thenReturn(Optional.empty());
  ...
  void getProviderFirms_withTypeFilter_returns200WithFilteredList() throws Exception {
    ...
    when(officeQueryService.getLspHeadOffice(entity1)).thenReturn(Optional.empty());
    when(officeQueryService.getChambersHeadOffice(entity1)).thenReturn(Optional.empty());
    when(officeQueryService.getAdvocateOfficeLink(entity1)).thenReturn(Optional.empty());
    when(providerFirmService.getParentLinks(entity1)).thenReturn(List.of());

    when(officeQueryService.getLspHeadOffice(entity2)).thenReturn(Optional.empty());
    when(officeQueryService.getChambersHeadOffice(entity2)).thenReturn(Optional.empty());
    when(officeQueryService.getAdvocateOfficeLink(entity2)).thenReturn(Optional.empty());
