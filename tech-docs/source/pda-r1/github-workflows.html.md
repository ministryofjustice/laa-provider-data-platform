---
source_url: https://github.com/ministryofjustice/laa-provider-data-platform/blob/main/tech-docs/source/pda-r1/github-workflows.html.md
---
...
### "Pipeline: PR opened/updated" ([`pipeline-pdl-pr-updated.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/pipeline-pdl-pr-updated.yml))
...
### "Pipeline: PR closed/merged" ([`pipeline-pdl-pr-closed.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/pipeline-pdl-pr-closed.yml))
...
The "Pipeline: PR closed/merged" workflow can also be invoked
...
![Screenshot of the "Pipeline: PR closed/merged" workflow dispatch form](./images/pipeline-pdl-pr-closed.png)

### "Pipeline: Branch main updated" ([`pipeline-pdl-main-updated.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/pipeline-pdl-main-updated.yml))
...
The "Pipeline: Branch main updated" workflow can also be invoked
...
![Screenshot of the "Pipeline: Branch main updated" workflow dispatch form](./images/pipeline-pdl-main-updated.png)
...
### "Reusable workflow: Gradle build and publish" ([`rw-gradle-build-and-publish.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/rw-gradle-build-and-publish.yml))
...
On `main`, it also publishes library artefacts to GitHub Packages. Checkstyle, unit test,
integration test, and JaCoCo coverage reports are uploaded as workflow artifacts.
...
### "Reusable workflow: ECR publish image" ([`rw-ecr-publish-image.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/rw-ecr-publish-image.yml))

Builds the OCI image using `./gradlew bootBuildImage` and pushes it to Amazon ECR with an
immutable tag (e.g. `pdl-v1.4.2` or `pdl-pr-999-c033174`) and an optional mutable tag (e.g.
`pdl-pr-999`, updated on each push to the PR).
...
![Screenshot of the "Reusable workflow: ECR publish image" workflow dispatch form](./images/rw-pdl-build.png)

### "Reusable workflow: Deploy PR" ([`rw-pdl-deploy-pr.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/rw-pdl-deploy-pr.yml))
...
The "Reusable workflow: Deploy PR" workflow can also be invoked
...
![Screenshot of the "Reusable workflow: Deploy PR" workflow dispatch form](./images/rw-pdl-deploy-pr.png)

### "Reusable workflow: Deploy main" ([`rw-pdl-deploy-main.yml`](https://github.com/ministryofjustice/laa-data-provider-data/blob/main/.github/workflows/rw-pdl-deploy-main.yml))
...
The "Reusable workflow: Deploy main" workflow can also be invoked
manually (as `workflow_dispatch`) in case, for example, you want to deploy a
...
![Screenshot of the "Reusable workflow: Deploy main" workflow dispatch form](./images/rw-pdl-deploy-main.png)
...
| `rw-gradle-build-and-publish.yml` | **Reusable workflow: Gradle build and publish** | Called by both `pipeline-pdl-*-updated.yml` pipelines |
| `rw-ecr-publish-image.yml`        | **Reusable workflow: ECR publish image**        | Called by both `pipeline-pdl-*-updated.yml` pipelines |
...
This section explains how to manually run the "Reusable workflow:
...
3. In the workflow dispatch form that opens from the menu button, select your PR branch
   (this ensures the workflow checks out your PR’s code). Now fill in the form fields:
...
![Screenshot of the "Reusable workflow: End-to-end test" workflow dispatch form](./images/rw-pdl-e2etest.png)
...
- Manually run the "Pipeline: PR closed/merged" workflow to clean
