...
> If you know these patterns, skip to [Options](#options).
...
All three options below share the following.
...
**Outbox payload as full snapshot** (Options 1 and 2) - if each outbox event payload carries the
complete aggregate state after the change (not just a delta), the outbox is also a point-in-time
history. Reconstruction is a single query: find the latest outbox row for the aggregate before the
target timestamp. The `created_at` timestamp records when the change occurred. `published_at`
records when it was forwarded to the broker and should not be used as the event time.
...
Option 3 (direct publish) leaves no persistent record in the service. Audit would need a separate
mechanism - Envers, a dedicated `audit_log` table, or capturing events at the broker.

Full event sourcing is the strongest form but adds significant complexity. If audit needs grow
beyond what Envers covers, it's an option to consider.
...
the `EventPublisher` port and outbox adapter slot into hexagonal, onion, and clean equally. Option
2 is the one exception: if using hexagonal, the command queue worker fits as `adapter/in/queue`
alongside `adapter/in/web`.

### Option 1: Transactional outbox (synchronous API)
...
|---|---|---|---|
| `EventPublisher` interface | `application/port/out` | `domain/service` | `usecase/boundary` |
| Outbox writer adapter | `adapter/out/event` | `infrastructure/event` | `adapter/event` |
...
### Option 2: Async command queue + outbox
...
A status endpoint (e.g. `GET /provider-firms/{id}`) lets callers check whether a command has been
processed.
...
### Option 3: Direct publish (simplest)
...
Steps are the same regardless of which option is chosen. Only Option 2 has additional steps.
...
4. Add the `outbox` table migration and implement the outbox writer adapter (Options 1 and 2).
5. Deploy the background job to read and publish outbox events (Options 1 and 2).
6. (Option 2) Add the `command_inbox` table, update the web adapter to write commands to it,
