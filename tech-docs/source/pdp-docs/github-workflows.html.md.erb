...
The provider data API (persistent)'s CI/CD process is implemented using GitHub Actions.
Deployment into Cloud Platform is implemented using `helm` and a Helm chart.

## Pipeline workflows

- The `pipeline-pdp-pr-*.yml` workflows automatically build, deploy and clean
...
- The `pipeline-pdp-main-updated.yml` workflow automatically versions, builds
...
![Screenshot of the "Pipeline: Branch main updated" workflow dispatch form](./images/pipeline-pdp-main-updated.png)
...
- **Optional release name**: Leave this blank for now

![Screenshot of the "Reusable workflow: Deploy main" workflow dispatch form](./images/rw-pdp-deploy-main.png)
...
2. `pipeline-pdp-pr-updated.yml` runs: it builds and pushes Docker image tags
...
5. `pipeline-pdp-pr-closed.yml` runs to clean up the preview app.

### Merge to `main`

1. Developer merges a PR to `main`.

2. `pipeline-pdp-main-updated.yml` checks commit messages:
...
executes end-to-end (E2E) tests for the provider data API (persistent)
against a release in a chosen namespace (**uat**, **staging**, **prod**) or a
PR preview app in **uat**.

It runs the test suite defined in `/provider-data-e2e` using **JUnit** and
...
4. Port-forwards the API service (e.g. `pdp-provider-data-platform` or
   `pdp-pr-913-provider-data-platform`) to `localhost:18080`.
...
> 💡 Example: if **rel** = `pr-913`, the workflow targets the service named `pdp-pr-913-provider-data-platform`.
...
![Screenshot of the "Reusable workflow: End-to-end test" workflow dispatch form](./images/rw-pdp-e2etest.png)
