---
source_url: https://github.com/ministryofjustice/laa-provider-data-platform/blob/main/tech-docs/source/pdp-docs/spring-boot-4-migration.html.md
...
This document describes the historical changes made to migrate the provider data API (persistent)
...
- Java 8 time types (`java.time.*`) are built into Jackson 3 core (no separate
  `jackson-datatype-jsr310` needed)
- The `com.fasterxml.jackson.*` (Jackson 2) → `tools.jackson.*` (Jackson 3) migration can happen
  gradually
...
The test code structure (use of annotations and test patterns) was already Spring Boot 4.0
compliant:
